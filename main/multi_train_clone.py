...
from dit_clone import DiffusionTransformer
from condition.t5condition import T5Conditioner
from train.validation_clone import validate_or_test
...
from train.utils import draw_plot, cleanup_memory, save_concatenated_mel_spectrogram, calculate_targets, prepare_batch_data, masked_loss, make_masked_audio
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load(f'./weights_0208_clone_full2/model_0_end.pth'), strict=False)
    cleanup_memory()
    ...
    output_dir   = 'weights_0208_clone_full2'
    ...
    scheduler = CosineAnnealingWarmupRestarts(
        optimizer,
        first_cycle_steps=50,  # 약 30 epoch마다 lr 감소
        cycle_mult=1.0,
        max_lr=lr,
        min_lr=lr/10,
        warmup_steps=5,
        gamma=0.4  # 사이클마다 max lr의 80%
    )
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (audio, info) in enumerate(train_loader):
            ...
            z_0, input_ids, attention_mask, mean_pooled = prepare_batch_data(
            ...
            loss = masked_loss(output, targets, maskeds, 'mse') # mask가 1인 부분에 대해서만 Loss 계산
            # if dist.get_rank() == 0:
            #     print("loss - ", loss)
            ...
            if idx % 50 == 49 and dist.get_rank() == 1:
            ...
            if idx % 500 == 499 and dist.get_rank() == 1:
                scheduler.step()
        ...
        print(f"=== End of epoch {epoch}, calling scheduler.step() === {dist.get_rank()}")
        if dist.get_rank() == 1:
            scheduler.step()
        ...
        if epoch % save_epoch == save_epoch - 1 and dist.get_rank() == 1:
            torch.save(model.module.state_dict(), f'./{output_dir}/model_{epoch}_end.pth')
        ...
        sfx_valid_loss = None
        model.eval()
        if dist.get_rank() == 1 and valid_loader is not None:
