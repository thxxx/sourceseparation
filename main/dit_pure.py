...
class DiffusionTransformer(nn.Module):
    ...
    def get_context(self, input_ids, attention_mask, seconds_start, seconds_total):
        ...
        if len(self.cross_attention_inputs_keys) > 0:
            ...
            for key in self.cross_attention_inputs_keys:
                ...
                cross_attention_inputs.append(cross_attn_in.to(model_dtype))
                cross_attention_masks.append(cross_attn_mask.to(model_dtype))
            cross_attention_inputs = torch.cat([x.to(model_dtype) for x in cross_attention_inputs], dim=1)
    ...
    def _forward(
        ...
        timestep_embed = self.to_timestep_embed(time_feat)
        ...
        cross_attention_inputs = cross_attention_inputs[:, :-2, :]
        cross_attention_masks = cross_attention_masks[:, :-2]
    ...
    def forward(self, x, t, mask, input_ids, attention_mask, seconds_start, seconds_total, prefix_cond=None, cfg_dropout_prob=0.1, cfg_scale=None, prepend_inputs=None, scale_phi=0.75):
